    ...
    const shaderSource = {
        ...
        present: `
            ...
            void main () {
                ...
                vec2 gradientOffset = uv - gradientCenter;
                gradientOffset.x *= 0.82;
                float blueField = 1.0 - smoothstep(0.12, 0.72, length(gradientOffset));
                float rightWash = smoothstep(0.14, 1.0, uv.x);
                float lowerGlow = 1.0 - smoothstep(
                    0.08,
                    0.62,
                    length(uv - vec2(0.62 + cos(time * 0.16) * 0.08, 0.08))
                );
