...
(() => {
    ...
    const shaderSource = {
        ...
        present: `
            ...
            uniform float clearingStrength;
            uniform float clearingRadius;

            float readPigmentPeak(vec2 sampleUv) {
                vec3 sampleColor = texture2D(pigmentMap, clamp(sampleUv, 0.0, 1.0)).rgb;
            ...
            void main () {
                ...
                vec2 backgroundUv = clamp(
                    uv - flowOffset * distortionStrength * interactionEnabled,
                    0.0,
                    1.0
                );
                ...
                vec2 whitePocketA = backgroundUv - vec2(
                    0.75 + sin(time * 0.13) * 0.08,
                    0.78 + cos(time * 0.17) * 0.05
                );
                whitePocketA.x *= 1.45;
                vec2 whitePocketB = backgroundUv - vec2(
                    0.92 + cos(time * 0.10) * 0.05,
                    0.43 + sin(time * 0.14) * 0.09
                );
                whitePocketB.x *= 1.7;
                vec2 whitePocketC = backgroundUv - vec2(
                    0.68 + sin(time * 0.09) * 0.07,
                    0.17 + cos(time * 0.12) * 0.04
                );
                whitePocketC.x *= 1.25;
                ...
                vec2 clearingStep = pixelStep * clearingRadius;
                float clearingDensity = max(
                    visibility,
                    max(
                        max(
                            readPigmentPeak(uv + vec2(clearingStep.x, 0.0)),
                            readPigmentPeak(uv - vec2(clearingStep.x, 0.0))
                        ),
                        max(
                            readPigmentPeak(uv + vec2(0.0, clearingStep.y)),
                            readPigmentPeak(uv - vec2(0.0, clearingStep.y))
                        )
                    )
                );
                float clearingAmount = smoothstep(0.03, 0.62, clearingDensity)
                    * clearingStrength
                    * interactionEnabled;
                backgroundColor = mix(backgroundColor, vec3(1.0), clearingAmount);
                vec3 originalGlowColor = clamp(pigment, 0.0, 1.0);
                float opacity = visibility * 0.54;
                gl_FragColor = vec4(
                    mix(backgroundColor, originalGlowColor, opacity),
                    1.0
                );
            }
        `,
    ...
    function displayPigment(currentTime) {
        ...
        gl.uniform1f(pass.uniform.clearingStrength, backgroundInteraction.clearingStrength);
        gl.uniform1f(pass.uniform.clearingRadius, backgroundInteraction.clearingRadius);
        bindTexture(pass.uniform.flowMap, fields.flow.read);
    ...
    function updateAutomaticFlow(currentTime) {
        ...
        automaticFlow.activeStrokes = automaticFlow.activeStrokes.filter((stroke) => {
            ...
            addImpulse(
                point[0],
                point[1],
                (point[0] - stroke.previousPoint[0]) * canvas.width * 8,
                (point[1] - stroke.previousPoint[1]) * canvas.height * 8,
                sampleBlueGradient((stroke.colorOffset + progress * 0.7) % 1),
                stroke.spread
            );
